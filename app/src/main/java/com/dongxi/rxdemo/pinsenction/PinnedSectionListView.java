...
import com.dongxi.rxdemo.BuildConfig;
...
public class PinnedSectionListView extends ListView {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (isFirstVisibleItemSection) {
                ...
                if (sectionView.getTop() == getPaddingTop()) { // view sticks to the top, no need for pinned shadow
                    ...
                    ensureShadowForPosition(firstVisibleItem, firstVisibleItem, visibleItemCount);  // 设置悬浮+阴影
                }
                ...
                } else { // there is no section for the first visible topic_new_item, destroy shadow
                    destroyPinnedShadow();
                }
            }
        }
     ...
     * 数据集观察者
     * 当数据集有变化时，会调用DataSetObserver的onChanged()方法；
     * 当数据集失效时，会调用DataSetObserver的onInvalidated()方法。
     */
    ...
    void ensureShadowForPosition(int sectionPosition, int firstVisibleItem, int visibleItemCount) {
        if (visibleItemCount < 2) { // no need for creating shadow at all, we have a single visible topic_new_item
            destroyPinnedShadow();
            return;
        }
    ...
    int findCurrentSectionPosition(int fromPosition) {
        ...
        // try slow way by looking through to the next section topic_new_item above
    ...
    private boolean isPinnedViewTouched(View view, float x, float y) {
        ...
        // by taping top or bottom padding, the list performs on click on a border topic_new_item.
